...
app.post('/api/analyze', async (req, res) => {
  ...
  try {
    const response = await fetch("https://ark.cn-beijing.volces.com/api/v3/chat/completions", {
      ...
      body: JSON.stringify({
        model: process.env.VISION_ENDPOINT_ID,
        messages: [
          {
            role: "user",
            content: [
    ...
    if (!response.ok) {
        ...
        return res.status(response.status).json({ error: `Volcengine Vision API Error: ${err}` });
    }
...
app.post('/api/generate', async (req, res) => {
  ...
  try {
    const response = await fetch("https://ark.cn-beijing.volces.com/api/v3/images/generations", {
      ...
      body: JSON.stringify({
        model: process.env.SEEDREAM_ENDPOINT_ID,
        prompt: prompt,
        image: originalImage,
        size: "2K",
    ...
    if (!response.ok) {
        const err = await response.text();
        return res.status(response.status).json({ error: `Volcengine API Error: ${err}` });
    }

    const data = await response.json();
    if (data.data && data.data.length > 0 && data.data[0].b64_json) {
        return res.status(200).json({ image: `data:image/jpeg;base64,${data.data[0].b64_json}` });
    }
    
    return res.status(500).json({ error: "未能生成图像" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: error.message });
  }
